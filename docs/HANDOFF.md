...
Snapshot 2026-09-07 from `main` @ `d3f8d5b`. Dated facts age; `PLINTH_SPEC.md`
...
- **Merged:** T-P1 scaffold + live URL; T-P2 parametric device set (five
  presets: `phone`, `tablet`, `laptop`, `browser`, `card`); T-P3 screenshot
  to screen; T-P4 studio lighting + materials (PR #5, reviewed; P-6/P-7
...
- **Side branches** on the remote besides `main`:
  `claude/scaffold-live-url-3pauwl`, `claude/shared-document-s75zk6`,
  `claude/t-p4-plinth-spec-research-vbonhx`. Check whether each is merged
  before assuming anything on them is live.
...
| # | Ticket | Cites | Note |
|---|---|---|---|
| T-P5 | Camera + posing: constrained orbit, `damp()` transitions, 4 poses, `aspectFix` | §4.3 | **next**; opens with a research pass |
...
3. Report in one page: what is done, what deviates from this brief, and
   what the next research pass must cover (the § clauses by number and
   the `file:symbol` surfaces). Do not write code and do not write the
