...
import { compositionSettings, type CompositionId } from './ui/compositions';
...
export interface Settings {
  ...
  fit: FitMode; pad: number; padColor: string; composition: CompositionId | null;
}
...
export function createSettingsStore(stage: Stage, studio: Studio, options: { immediate: boolean; msaa: boolean; fixedAspect?: number }): SettingsStore {
  let state: Settings = { ...stage.snapshot(), aspect: '4:5', scene: stage.getScene(), tone: studio.getToneMapping(),
    msaa: options.msaa, background: defaultBackground(), composition: null };
  if (!options.fixedAspect && state.device === 'phone' && state.scene === 'soft-studio' && state.pose === 'hero' && !state.msaa) state.composition = 'studio-phone';
  ...
  const unsubscribe = stage.onStateChange(reason => {
    ...
    if (!['advancePose', 'setImage'].includes(reason)) state.composition = null;
    emit(reason);
  });
  const api: SettingsStore = {
    ...
    apply(patch, composition) {
      ...
      const stageCandidate = stage.prepareSettings({ ...next, aspect: options.fixedAspect ?? ratio }, options.immediate,
    ...
    reset() { api.compose('studio-phone'); },
